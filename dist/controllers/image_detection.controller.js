...
function isSocialSize(w, h) {
    if (!w || !h)
        return false;
    return SOCIAL_SIZES.some(([sw, sh]) => sw !== undefined && sh !== undefined &&
        (Math.abs(sw - w) < 10 && Math.abs(sh - h) < 10));
}
async function detectBlur(path) {
...
async function checkExif(path) {
    const exif = await exiftool_vendored_1.exiftool.read(path);
    let penalty = 0;
    let bonus = 0;
    let reasons = [];
...
async function checkReuse(path) {
    const hash = await imghash_1.default.hash(path, 16);
    /*   const existing = await prisma.productImage.findMany({
          select: { phash: true },
      }); */
    const existing = await prisma_js_1.prisma.productImage.findMany({
        where: {
...
/* =====================
   5. SCORING ENGINE
   ===================== */
async function sellerTrustBonus(userId) {
    const approved = await prisma_js_1.prisma.productImage.count({
...
async function scoreImage(path, userId) {
...
async function resolveImagePath(input) {
    if (!/^https?:\/\//i.test(input)) {
        return { path: input, cleanup: async () => { } };
    }
    const response = await axios_1.default.get(input, { responseType: "arraybuffer" });
    const tmpPath = path_1.default.join(os_1.default.tmpdir(), `img-${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`);
    await promises_1.default.writeFile(tmpPath, Buffer.from(response.data));
    return {
        path: tmpPath,
        cleanup: async () => {
...
// });
//# sourceMappingURL=image_detection.controller.js.map